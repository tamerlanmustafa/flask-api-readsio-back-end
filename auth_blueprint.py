...
def sign_up():
    try:
        ...
        cursor.execute("INSERT INTO users (username, password_hash) VALUES (%s, %s) RETURNING id, username", (new_user_data["username"], hashed_password.decode('utf-8')))
...
def sign_in():
    try:
        ...
        if not sign_in_form_data:
            return jsonify({"err": "Request body is empty or not valid JSON."}), 400
        if 'username' not in sign_in_form_data or 'password' not in sign_in_form_data:
            return jsonify({"err": "'username' or 'password' is missing in the request."}), 400
        connection = get_db_connection()
        cursor = connection.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        ...
        password_is_valid = bcrypt.checkpw(bytes(sign_in_form_data["password"], 'utf-8'), bytes(existing_user["password_hash"], 'utf-8'))
